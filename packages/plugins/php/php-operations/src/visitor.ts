...
import { getListInnerTypeNode, PHP_SCALARS, getListTypeField, getListTypeDepth } from '../../common/common';
...
export interface PhpOperationsPluginConfig extends ClientSideBasePluginConfig {
...
export class PhpOperationsVisitor extends ClientSideBaseVisitor<
  PhpOperationsRawPluginConfig,
  PhpOperationsPluginConfig
> {
  ...
  constructor(
    ...
    rawConfig: PhpOperationsRawPluginConfig,
    ...
    super(
      ...
      rawConfig,
      {
        ...
        scalars: buildScalars(schema, rawConfig.scalars, PHP_SCALARS),
      },
      documents
    );
  ...
  // Some settings aren't supported with PHP, overruled here
  private overruleConfigSettings() {
    if (this.config.documentMode === DocumentMode.graphQLTag) {
      // PHP operations does not (yet) support graphQLTag mode
